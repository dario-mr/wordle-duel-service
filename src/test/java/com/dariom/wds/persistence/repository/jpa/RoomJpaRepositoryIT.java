...
class RoomJpaRepositoryIT {
  ...
  void findWithPlayersById_roomWithPlayers_returnsLoadedElementCollections() {
    ...
    var scores = found.getMatchScoresByPlayerId();
  ...
  void findWithPlayersByPlayerId_playerInMultipleRooms_returnsRoomsOrderedByLastUpdatedAtDesc() {
    ...
    room1.setPlayerMatchScore("p2", 1);
    ...
    room2.addPlayer("p1");
    room2.setPlayerMatchScore("p1", 5);
    ...
    roomOther.setPlayerMatchScore("p2", 0);
    ...
    assertThat(found.get(0).getMatchScoresByPlayerId()).containsEntry("p1", 5);
  ...
  void deleteInactive_oldRoomExists_deletesOnlyOldRooms() {
    ...
    var establishedRoom = new RoomEntity();
    establishedRoom.setId("room-established");
    establishedRoom.setLanguage(IT);
    establishedRoom.setStatus(IN_PROGRESS);
    establishedRoom.addPlayer("p1");
    establishedRoom.addPlayer("p2");

    repository.save(oldRoom);
    repository.save(newRoom);
    repository.save(establishedRoom);
    entityManager.flush();
    ...
    entityManager.createNativeQuery("update rooms set last_updated_at = :ts where id = :id")
        .setParameter("ts", oldTs)
        .setParameter("id", "room-established")
        .executeUpdate();
    ...
    assertThat(repository.findById("room-established")).isPresent();
  }
